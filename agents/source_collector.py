...
def _load_requirements() -> Tuple[List[Dict[str, Any]], Dict[str, List[Dict[str, Any]]]]:
    payload = json.loads(_REQUIREMENTS_PATH.read_text(encoding="utf-8"))
    return payload["common"], payload["models"]
...
def build_source_requirements(ticker: str, model_id: str) -> Dict[str, Any]:
    ...
    common_requirements, requirements_by_model = _load_requirements()

    if normalized_model not in requirements_by_model:
        raise ValueError(f"Unsupported model_id: {model_id}")

    common = [dict(item) for item in common_requirements]
    model_specific = [dict(item) for item in requirements_by_model[normalized_model]]
    return {
        "ticker": ticker.strip().upper(),
        "model_id": normalized_model,
        "source_requirements": common + model_specific,
    }
